...
case object Empty extends MyList[Nothing] {
  ...
  //
  def map[B] (transformer: MyTransform[Nothing, B]): MyList[B] = Empty
  def flatMap[B](transformer: MyTransform[Nothing, MyList[B]]): MyList[B] = Empty
...
object ListTest extends App {
  ...
  */
  ...
  val listTransform = listOfInterger.map(new MyTransform[Int, Int] {
    override def transform(element: Int): Int = element * 2
  })

  println(listOfInterger.toString)

  val listPredicate = listOfInterger.filter(new MyPredicate[Int] {
  ...
  val listFilterMap = listOfInterger.flatMap(new MyTransform[Int, MyList[Int]] {
